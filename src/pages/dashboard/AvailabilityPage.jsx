...
const AvailabilityPage = () => {
    ...
    fetchRules().then((data) => {
      if (data?.slots) {
        ...
        DAYS_OF_WEEK.forEach((day, index) => {
          const slot = data.slots.find(s => s.day_of_week === index);
  ...
  const handleSave = async () => {
    ...
    try {
      // 🔥 Convert schedule → backend format
      const slots = [];
      ...
      console.log("🔥 FINAL PAYLOAD:", { slots });

      await updateRules({ slots }); // ✅ CORRECT
      ...
      <Card>
        <CardContent className="divide-y divide-gray-200 p-0">
          {DAYS_OF_WEEK.map((day) => {
            ...
            return (
              <div key={day} className="flex items-center justify-between p-4 sm:p-6 hover:bg-gray-50">
